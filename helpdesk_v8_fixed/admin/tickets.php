...
$tickets = $conn->query("
    SELECT t.*, k.nama_kategori, jp.nama AS jenis_pengirim_nama,
    ...
    <div class="main-content">
        ...
        <main class="page-content">
            <form method="GET" class="filter-bar">
                ...
                <a href="tickets" class="btn btn-outline btn-sm">Reset</a>
            </form>
            ...
            <div class="card">
                ...
                <table>
                    <thead>
                        <tr><th>#</th><th>Kode</th><th>Pelapor</th><th>Status Pengirim</th><th>Judul</th><th>Kategori</th><th>Prioritas</th><th>Status</th><th>Masuk</th><th>Aksi</th></tr>
                    </thead>
                    <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $jp = $row['jenis_pengirim_nama'] ?? '';
                            if (!empty($jp)):
                                $jpLow = strtolower($jp);
                                if (str_contains($jpLow,'kirim') || str_contains($jpLow,'sent') || str_contains($jpLow,'terkirim')) {
                                    $jpColor = 'background:#dcfce7;color:#166534';
                                } elseif (str_contains($jpLow,'pending') || str_contains($jpLow,'tunggu')) {
                                    $jpColor = 'background:#fef9c3;color:#854d0e';
                                } elseif (str_contains($jpLow,'gagal') || str_contains($jpLow,'fail') || str_contains($jpLow,'error')) {
                                    $jpColor = 'background:#fee2e2;color:#991b1b';
                                } else {
                                    $jpColor = 'background:#e0f2fe;color:#0369a1';
                                }
                            ?>
                            <span style="<?= $jpColor ?>;padding:2px 8px;border-radius:12px;font-size:.7rem;font-weight:600"><?= htmlspecialchars($jp) ?></span>
                        ...
                        <td>
                            <a href="ticket_detail?uid=<?= urlencode($row['uid']) ?>" class="btn btn-sm btn-primary">💬 Chat</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                    <?php if ($total === 0): ?>
